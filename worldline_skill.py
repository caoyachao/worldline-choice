...
class ActionAnalysis:
    ...
    detected_npcs: List[str] = None  # 涉及的NPC名称列表
...
class NarrativeContext:
    ...
    detected_npcs: List[str] = None  # 本回合涉及的NPC
...
class GameState:
    ...
    def __init__(self):
        ...
        # 战术增强系统已移除（v4.7.0）：由LLM在叙事中处理环境互动和NPC协作
...
"""
    ...
    def _default_options(self, state: GameState) -> Dict:
        """默认选项（无LLM时的极简回退）"""
        return {
            "context": f"当前场景：{state.current_scene}",
            "options": [
                {
                    "letter": "A",
                    "description": "谨慎行动",
    ...
    def _default_narrative(self, ctx: NarrativeContext) -> Dict:
        ...
        return {
            "narrative": templates.get(ctx.check_result.degree, "行动结束。"),
            "consequences": {
                "attribute_changes": {},
...
class WorldlineSkill:
    ...
    def start_game(
        ...
        # 金钱系统：由LLM在世界观初始化时设定，不再硬编码
        self.state.money_per_turn = 0
