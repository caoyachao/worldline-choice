...
> v4.5.0 存档结构：每个游戏拥有独立的 `game_id` 目录，如 `saves/game_1780885435_武侠/game.json`，支持多游戏存档隔离。
...
- **v4.5.0** (2026-06-08): 角色成长版。引入角色成长系统：HP/资源/背包/状态效果/属性历史审计；回合结算引擎（tick衰减→修正计算→体质联动→死亡检测）；游戏目录隔离；属性上限50。
