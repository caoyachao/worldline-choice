...
description: |
  AI驱动的互动叙事游戏引擎 v4.6.0 - DC校准与收益版。作为主持人(agent)，你与玩家共同创造一个开放式的故事世界。
  核心职责：初始化游戏→生成场景和ABCD+E选项→接收玩家选择→调用引擎执行d20检定→根据骰子结果讲述剧情→更新状态。
  本skill强制透明暴露d20结果，禁止LLM脑补骰子结果；引擎层强制执行每回合自动保存，禁止LLM幻觉保存行为。
  v4.6.0新增DC校准与收益系统：DC重新校准（简单2-7/中等8-12/困难13-17），收益与DC严格对应，统一存档系统（单文件+事件压缩），完整NPC关系系统。
---

# Worldline Choice — Agent 主持手册 (v4.6.0)
...
每回合 `process_turn` 结束后，引擎自动执行 `settle_turn()`，四阶段：
...
4. **死亡检测**：`HP <= 0` 时标记 `death_triggered`，后续回合拒绝处理，返回结局叙事
...
旧的 `relationship_changes` 仍支持（向后兼容），但推荐使用完整结构。
...
> v4.5.0 存档结构：每个游戏拥有独立的 `game_id` 目录，如 `saves/game_1780885435_武侠/game.json`，支持多游戏存档隔离。NPC 关系数据包含完整的 15 字段社交网络模型（关系、信任、恐惧、忠诚、好感、声誉、互动历史、记忆、阵营等）。
